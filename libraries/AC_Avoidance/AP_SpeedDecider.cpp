...
#include <AP_PiecewiseJerk/piecewise_jerk_speed_problem.h>
...
const AP_Param::GroupInfo AP_SpeedDecider::var_info[] = {
  ...
  // @Param: SPD_MAX
  // @Description: Maximum acceleration of vehicle
  // @Units: m/s^2
  // @Range: 0 10
  // @User: Standard
  AP_GROUPINFO("SPD_MAX", 1, AP_SpeedDecider, _speed_max, 3.0f),
  ...
  // @Range: 0 10
  // @User: Standard
  AP_GROUPINFO("ACC_MAX", 2, AP_SpeedDecider, _acc_max, 0.3f),
  ...
  // @Range: 0 10
  // @User: Standard
  AP_GROUPINFO("DEC_MAX", 3, AP_SpeedDecider, _dec_max, 0.5f),

  // @Param: JERK_MAX
  // @Description: Maximum JERK of vehicle
  // @Units: m/s^3
  // @Range: 0 10
  // @User: Standard
  AP_GROUPINFO("JERK_MAX", 4, AP_SpeedDecider, _jerk_max, 1.0f),
  ...
  // @Range: 0.1 10
  // @User: Standard
  AP_GROUPINFO("ROB_RADIUS", 5, AP_SpeedDecider, _vehicle_radius, 1.0f),
  ...
  // @Range: 1 10
  // @User: Standard
  AP_GROUPINFO("UNT_TIME", 8, AP_SpeedDecider, _unit_t, 1.0f),
  ...
  // @Range: 0.1 10
  // @User: Standard
  AP_GROUPINFO("UNT_DENS", 9, AP_SpeedDecider, _dense_unit_s, 0.2f),
  ...
  // @Range: 1 10
  // @User: Standard
  AP_GROUPINFO("UNT_SPAS", 10, AP_SpeedDecider, _sparse_unit_s, 1.0f),

  // @Param: PLN_MIN
  // @Description: minimal Planning distance
  // @Units: m
  // @Range: 1 100
  // @User: Standard
  AP_GROUPINFO("PLN_MIN", 11, AP_SpeedDecider, _planning_length_min, 15.0f),

  // @Param: OBS_WGH
  // @Description: Obstacle penalty factor
  // @Range: 1 1e10
  // @User: Standard
  AP_GROUPINFO("OBS_WGH", 12, AP_SpeedDecider, _obstacle_weight, 1.0f),

  // @Param: ACC_WGH
  // @Description: Acceleration penalty factor
  // @Range: 1 1e10
  // @User: Standard
  AP_GROUPINFO("ACC_WGH", 13, AP_SpeedDecider, _accel_penalty, 10.0f),

  // @Param: DEC_WGH
  // @Description: Dcceleration penalty factor
  // @Range: 1 1e10
  // @User: Standard
  AP_GROUPINFO("DEC_WGH", 14, AP_SpeedDecider, _decel_penalty, 10.0f),

  // @Param: DEC_WGH
  // @Description: Jerk penalty factor
  // @Range: 1 1e10
  // @User: Standard
  AP_GROUPINFO("JERK_WGH", 15, AP_SpeedDecider, _jerk_penalty, 10.0f),

  // @Param: RV_WGH
  // @Description: reference velocity(RV) penalty factor
  // @Range: 1 1e10
  // @User: Standard
  AP_GROUPINFO("RV_WGH", 16, AP_SpeedDecider, _ref_v_weight, 10.0f),

  // @Param: RS_WGH
  // @Description: reference s(RS) penalty factor
  // @Range: 1 1e10
  // @User: Standard
  AP_GROUPINFO("RS_WGH", 17, AP_SpeedDecider, _ref_s_weight, 10.0f),
                             ...
                             const Vector2f &ground_speed_vec, float &desired_speed_new, const float dt) 
{
   if (!_enable) {
    ...
    _curr_end = _curr_start +  projected_line_unit * _planning_length;

    // could not speed planning, then return not required
    if (distance_to_end  < MIN(_planning_length_min, _planning_length)) {
      return false;
    }
    ...
    // if enviroment do not have dynamical obstacles, speed regulator will disable
    if (_st_boundaries.empty()) {
      return false;
    }
    ...
    // piecewise jerk speed optimizer
    if (!piecewise_jerk_speed_optimizer(&speed_data)) {
       GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "Failed to optimizer speed QP");
    }
 ...
 bool AP_SpeedDecider::piecewise_jerk_speed_optimizer(planning::SpeedData* const speed_data)
 {
    // Set st problem dimensions
    const double delta_t = 1.0;
    const double total_length = speed_data->speed_vector().back().s();
    const double total_time = speed_data->TotalTime();
    const int num_of_knots = static_cast<int>(total_time / delta_t) + 1;

    // Set initial values
    // TODO: initial accel ??
    std::array<double, 3> init_s = {0.0f, _curr_speed, 0.0};

    // construct QP solver
    planning::PiecewiseJerkSpeedProblem piecewise_jerk_problem(num_of_knots, delta_t, init_s);

    // set penalty weight
    piecewise_jerk_problem.set_weight_ddx(_accel_penalty);
    piecewise_jerk_problem.set_weight_dddx(_jerk_penalty);

    // set 0<= s <= smax
    piecewise_jerk_problem.set_x_bounds(0.0, total_length);

    // set 0 <= v <= max(v0, vmax)
    piecewise_jerk_problem.set_dx_bounds(0.0f, std::fmax(_speed_max, _curr_speed));

    // set -dcc <= a <=  acc
    piecewise_jerk_problem.set_ddx_bounds(-_dec_max, _acc_max);

    // set -jerk <= jerk <= +jerk
    piecewise_jerk_problem.set_dddx_bound(-_jerk_max, _jerk_max);

    piecewise_jerk_problem.set_dx_ref(_ref_v_weight, _input_speed);
    ...
    for (int i = 0; i < num_of_knots; ++i) {
      ...
      for (auto &boundary : _st_boundaries) {
          // get coarse speed solution
          planning::SpeedPoint sp;
          if (!speed_data->EvaluateByTime(curr_t, &sp)) {
              GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "Failed to get speed by time 1");
              return false;
          }
          // get s_upper and s_lower
          float s_lower = 0.0;
          float s_upper = 0.0;
          if (boundary.get_boundary_srange(curr_t, &s_upper, &s_lower)) {
              // according ST boundary type to decider ocp boundarys
            if (sp.s() <= s_lower) {
              // STOP or YIELD or Follow
              s_upper_bound = std::fmin(s_upper_bound, std::fmax(s_lower, 0.0f));
            } else if (sp.s() >= s_upper) {
              // OVERTAKE
              s_lower_bound = std::fmax(s_lower_bound, std::fmin(s_upper, total_length));
            }
          }
      }
      ...
      if (s_lower_bound > s_upper_bound) {
        GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "s_lower_bound larger than s_upper_bound on STGraph");
        return false;
      }
    ...
    for (int i = 0; i < num_of_knots; ++i) {
      ...
      planning::SpeedPoint sp;
      if (!speed_data->EvaluateByTime(curr_t, &sp)) {
          GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "Failed to get speed by time 2");
          return false;
      }
    ...
    speed_data->Clear();
    speed_data->AppendSpeedPoint(s[0], 0.0f, ds[0], dds[0], 0.0f);
                                                   ...
                                                   std::vector<planning::STPoint>& upper_points)
{
  ...
  for (float curr_t = _planning_period; curr_t <= _total_t; curr_t += _planning_period) {
                                      ...
                                      const float cruise_speed) const
{
  ...
  if (_enable_dp_reference_speed) {
    ...
    cost += _ref_v_weight * default_speed_cost * fabs(diff_speed) * _unit_t;
  }
