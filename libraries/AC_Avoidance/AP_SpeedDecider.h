...
private:
    ...
    // QP optimize
    bool piecewise_jerk_speed_optimizer(planning::SpeedData* const speed_data);
...
private:
    ...
    AP_Float _speed_max;         // maximum speed
    AP_Float _acc_max;           // acceleration maximum value ,set must >= 0
    AP_Float _dec_max;           // deceleration maximum value, set must >= 0
    AP_Float _jerk_max;          // jerk maximum value, set must  >= 0
    ...
    AP_Float _dense_unit_s;      // dense distance scale >= 0.5f * acc * unit_t * unit_t
    AP_Float _sparse_unit_s;     // sparse distance scale >= 0.5f * acc * unit_t * unit_t
    ...
    AP_Float _jerk_penalty;      // jerk penalty
    AP_Float _ref_v_weight;      // cruise speed weight
    AP_Float _ref_s_weight;      // reference s weight
